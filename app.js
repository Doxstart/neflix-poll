...
startLoading()
DataService.getSeries().then(data => {addDataToCollection(data); console.log(seriesCollection); displayCollection(); stopLoading();})
.catch(err => {
    ...
    displayErrorMessage('Accidenti, si è verificato un errore'); stopLoading();
});
...
function upVoteClicked(serie){
    serie.upVotes += 1;
    startLoading();
    DataService.putSerie(serie).then(modifiedSerie => {stopLoading(); displayCollection()})
    .catch(error => {
        stopLoading();
...
function saveNewSerie(){
    ...
    DataService.postSerie(newSerie).then(savedSerie => {
        ...
        seriesCollection.addSerie(newSerie);
        displayCollection()
    }).catch(err => {stopLoading(); displayErrorMessage('Accidenti, non puoi salvare nuove serie')});
    // seriesCollection.addSerie(newSerie);
    // displayCollection();
}
